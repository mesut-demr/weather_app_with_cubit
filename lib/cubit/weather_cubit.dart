...
class WeatherCubit extends Cubit<WeatherState> {
  ...
  WeatherCubit(): super(WeatherState(status: WeatherStatus.loading)) {
       _init();
      filterCitites('');
  }
  ...
  Future<void> fetchWeatherForUserLocation() async {
    ...
    try {
      ...
      await _saveLocationToPrefs(currentPosition.latitude, currentPosition.longitude);
      await fetchWeatherForLocation(currentPosition.latitude, currentPosition.longitude);
    } catch (e) {
      emit(state.copyWith(status: WeatherStatus.errorMessage,errorMessage: 'Hava Durumu Alınamadı:$e'));
    }
  }

  Future<void> fetchWeatherForLocation(double latitude, double longitude) async {
    emit(state.copyWith(status: WeatherStatus.loading));
    try {
      WeatherModel newWeather = await weatherApiService.fetchWeatherForLocation(latitude, longitude);
      emit(state.copyWith(status: WeatherStatus.completed, weatherModel: newWeather));
    } catch (e) {
      emit(state.copyWith(status: WeatherStatus.errorMessage,errorMessage: 'Hava Durumu Alınamadı:$e'));
    }
  }
  ...
  void _startListeningLocationChanges() {
    _locationSubscription = locationService
        .listenLocationChanges().listen((Position position) async {
      WeatherModel newWeather =await weatherApiService.fetchWeatherForUserLocation();
      emit(state.copyWith(status: WeatherStatus.completed, weatherModel: newWeather),
      );
    });
  }
  ...
  Future<void> fetchWeatherForCity(String city) async {
    ...
    try {
      WeatherModel requestWeather =await weatherApiService.fetchWeatherForCity(city);
      emit(state.copyWith(status: WeatherStatus.completed, weatherModel: requestWeather));
    } catch (e) {
      emit(state.copyWith(status: WeatherStatus.errorMessage,errorMessage: 'Hava Durumu Alınamadı.'));
    }
  }
...
void setSearchButtonClicked(bool value){
